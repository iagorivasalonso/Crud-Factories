    ...
    try {
      switch (action) {
       ...
       case 'update':
         ...
         try {
              if (config.database?.trim().toLowerCase() === 'default') {
                return {
                  ok: false,
                  error: {
                    type: 'ProtectedDatabase',
                    code: 'DEFAULT_DATABASE',
                    message: 'La base de datos default no se puede editar'
                  }
                };
              }
           ...
           return result;
                  ...
                  console.error('ERROR AL RENOMBRAR LA BASE DE DATOS:', e);
           throw e;
         }
        case 'delete':
          ...
          if (config.database?.trim().toLowerCase() === 'default') {
            return {
              ...
              error: {
                type: 'ProtectedDatabase',
                code: 'DEFAULT_DATABASE',
                message: 'La base de datos default no se puede eliminar'
              }
            };
          }
