...
import { getPool, buildDbConfig } from '../repositories/database.pool.js';
...
export const handleDynamicQuery = async (req, res, sql, params = [], singleResult = false) => {
  ...
  if (!dbName || !host || !port || !user || !password) {
    return res.status(400).json({ error: "Faltan credenciales o base de datos" });
  }
