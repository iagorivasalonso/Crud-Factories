...
```js
var userId = 1;
var columns = ['username', 'address'];
var sql     = SqlString.format('SELECT ?? FROM ?? WHERE id = ?', [columns, 'users', userId]);
console.log(sql); // SELECT `username`, `address` FROM `users` WHERE id = 1
```
...
```js
var userId = 1;
var data   = { address: 'foobar@example.com', modified: SqlString.raw('NOW()') };
var sql    = SqlString.format('UPDATE ?? SET ? WHERE `id` = ?', ['users', data, userId]);
console.log(sql); // UPDATE `users` SET `address` = 'foobar@example.com', `modified` = NOW() WHERE `id` = 1
```
